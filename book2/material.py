...
class Material:
    ...
    def emitted(self, u: float, v: float, p: "Point3") -> "Color":
        '''
        Emmiting light if the material is glowing.

        Attributes:
            u: The x-coord of the image.
            v: The y-coord of the image.
            p: The hit point.

        Returns:
            Color: The emitted color.
...
class Lambertian(Material):
    ...
    def __init__(self, albedo: "Color" = None, texture: "Texture" = None) -> \
            None:
    ...
    def scatter(self, r_in: Ray, rec: HitRecord) -> Tuple[bool | Color | Ray]:
        ...
        return True, self.tex.value(rec.u, rec.v, rec.p),
        Ray(rec.p, scatter_direction, r_in.time())
...
class Dielectric(Material):
    ...
    def scatter(self, r_in: Ray, rec: HitRecord) -> Tuple[bool | Color | Ray]:
        ...
        if ri * sin_theta > 1.0 or \
                self._reflectance(cos_theta, ri) > random_float():
    ...
    def _refract(self, uv: "Vector3", n: "Vector3", etai_over_eta: float) -> \
            "Vector3":
